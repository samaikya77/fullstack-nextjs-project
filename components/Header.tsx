  ...
  const itemCount = useBasketStore((state)=>state.items.reduce((total,item)=>total+item.quantity,0));
    ...
    <div className='flex flex-wrap justify-between items-center px-4 py-2'>
      ...
      <div className='flex w-full flex-wrap justify-between items-center'>
        ...
        <input type='text'
         name='query' // name the input field to match the
        ...
        className='bg-gray-100 text-gray-800 px-4 py-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 border w-full max-w-4xl'/>
          ...
          <div className='flex items-center space-x-4 mt-2 sm:mt-0 flex-1 sm:flex-none'>
            ...
            <ClerkLoaded>
              {user&&(
                <Link href='/orders' className='relative flex items-center justify-center space-x-2 bg-blue-500
                hover:bg-blue-700 text-white font-bold py-2 px-4 rounded h-full'>
              ...
              {user ?.passkeys.length === 0 && (
                <button onClick={createClerkPasskey} className='bg-white hover:bg-blue-700 hover:text-white animate-pulse 
                text-blue-500 font-bold py-2 px-4 rounded border border-blue-500'>Create  PassKey  </button>
              )
              }
            </ClerkLoaded>
            </div>
