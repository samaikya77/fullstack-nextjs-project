...
export function CategorySelectorComponent({
  ...
  const [open, setOpen] = useState(false); //whether dropdown is open
  const [value, setValue]=useState(""); // currently selected category_id
  return (
              ...
              if (e.key === "Enter") {
                const selectedCategory = categories.find((c) =>
                  c.title
                    ?.toLowerCase()
                    .includes(
                      (e.currentTarget as HTMLInputElement).value.toLowerCase()
                    )
                );
